...
function mc_default_options() {
	$options = array(
		...
		'ajax_javascript'              => 0,
		'show_js'                      => '',
		'notime_text'                  => __( 'All day', 'my-calendar' ),
		'hide_icons'                   => 'true',
		...
		'view_full'                    => '',
		'week_caption'                 => '',
		'next_events'                  => __( 'Next', 'my-calendar' ),
		'previous_events'              => __( 'Previous', 'my-calendar' ),
		'today_events'                 => __( 'Today', 'my-calendar' ),
		'caption'                      => '',
		'month_format'                 => '',
 ...
 * @param string $parent Parent directory. Optional.
 *
 * @return boolean.
 */
function my_calendar_copyr( $source, $dest, $parent = '' ) {
	if ( ! WP_Filesystem() ) {
		exit;
	}
	...
	// If saving to a custom subdirectory, verify that parent directory exists.
	if ( $parent && ! is_dir( $parent ) ) {
		$wp_filesystem->mkdir( $parent );
	}
	if ( ! is_dir( $dest ) ) {
...
function my_calendar_backup( $process, $plugin ) {
	if ( isset( $plugin['plugin'] ) && 'my-calendar/my-calendar.php' === $plugin['plugin'] ) {
		$parent = plugin_dir_path( __DIR__ ) . '/my-calendar-custom/';
		$to     = plugin_dir_path( __DIR__ ) . '/my-calendar-custom/backup/';
		$from   = plugin_dir_path( __FILE__ ) . '/styles/';
		my_calendar_copyr( $from, $to, $parent );
	}
}
...
function my_calendar_recover( $process, $plugin ) {
	if ( isset( $plugin['plugin'] ) && 'my-calendar/my-calendar.php' === $plugin['plugin'] ) {
		$from = plugin_dir_path( __DIR__ ) . '/my-calendar-custom/backup/';
		$to   = plugin_dir_path( __FILE__ ) . '/styles/';
