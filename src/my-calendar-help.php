...
function my_calendar_help() {
	...
	<div class="wrap my-calendar-admin my-calendar-help">
...
function mc_contextual_help_css() {
	...
	if ( 'admin_page_mc-contextual-help' === get_current_screen()->id ) {
		wp_enqueue_style( 'mc-contextual-help', plugins_url( 'css/help.css', __FILE__ ), array(), $version );
	}
}
...
function mc_get_help_text( $id ) {
	$help = array(
		'1' => array(
			...
			'text'  => __( 'Create a duplicate copy of this event with a different date and time. After creation, you can manage each event separately in the event manager.', 'my-calendar' ),
		),
		'2' => array(
		...
		'8' => array(
			'title' => '',
			'text'  => mc_display_template_tags( false, 'code', 'classic' ),
		),
	);
