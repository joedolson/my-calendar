	...
	foreach ( $upgrade_path as $upgrade ) {
		switch ( $upgrade ) {
			...
			case '3.3.0':
				...
				$settings = array();
				$single   = mc_get_option( 'display_single', array() );
				$main     = mc_get_option( 'display_main', array() );
				$mini     = mc_get_option( 'display_mini', array() );
				if ( empty( $single ) || empty( $main ) || empty( $mini ) ) {
					$settings[] = ( 'true' === mc_get_option( 'mc_display_author' ) ) ? 'author' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_display_host' ) ) ? 'host' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_show_event_vcal' ) ) ? 'ical' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_show_gcal' ) ) ? 'gcal' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_show_map' ) ) ? 'gmap_link' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_gmap' ) ) ? 'gmap' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_show_address' ) ) ? 'address' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_short' ) ) ? 'excerpt' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_desc' ) ) ? 'description' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_image' ) ) ? 'image' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_event_registration' ) ) ? 'tickets' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_event_link' ) ) ? 'link' : '';
					$settings[] = ( 'true' === mc_get_option( 'mc_display_more' ) ) ? 'more' : '';
...
function mc_location_field_callback() {
	$value = ( '' !== get_option( 'mc_location_cpt_base', '' ) ) ? get_option( 'mc_location_cpt_base' ) : 'mc-locations';
