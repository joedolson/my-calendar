...
function mc_generate_css() {
	...
	if ( '' !== $style_vars ) {
		$style_vars = '.mc-main, .my-calendar-modal, .my-calendar-modal-overlay {' . $style_vars . $category_vars . '}';
	}
...
function mc_enqueue_duet() {
	$version = ( SCRIPT_DEBUG ) ? wp_rand( 10000, 100000 ) : mc_get_version();
	wp_enqueue_script( 'duet.js', plugins_url( 'js/duet/duet.js', __FILE__ ), array(), $version );
	wp_enqueue_style( 'duet.css', plugins_url( 'js/duet/themes/default.css', __FILE__ ), array(), $version );
	// Enqueue datepicker options.
	wp_enqueue_script( 'mc.duet', plugins_url( 'js/mc-datepicker.js', __FILE__ ), array( 'duet.js' ), $version, true );
	wp_localize_script(
		'mc.duet',
		'duetFormats',
		array(
