		...
		if ( $use_default || $js_usage || is_singular( 'mc-events' ) || is_singular( 'mc-locations' ) ) {
			...
			if ( 'true' === mc_get_option( 'mc_gmap' ) || mc_output_is_visible( 'gmap', 'single' ) || is_singular( 'mc-locations' ) ) {
				...
				if ( $api_key ) {
					...
					wp_enqueue_script( 'gmaps', "https://maps.googleapis.com/maps/api/js?v=3&libraries=places,marker&key=$api_key", array(), null, $args );
...
function mc_register_scripts() {
	...
	wp_register_style( 'mc-user-styles', plugins_url( 'css/mc-user-styles.css', __FILE__ ), array(), $version );
	...
	wp_register_script( 'mc.admin', $adminjs, array( 'jquery', 'jquery-ui-sortable', 'wp-a11y' ), $version, true );
	...
	if ( $api_key ) {
		...
		wp_register_script( 'gmaps', "https://maps.googleapis.com/maps/api/js?v=3&libraries=places,marker&key=$api_key", array(), null, $args );
...
function mc_admin_styles() {
	...
	if ( 'profile' === $id || 'user-edit' === $id || 'widgets' === $id || 'customize' === $id ) {
...
function mc_scripts() {
	...
	if ( $slug . '_page_my-calendar-design' === $id || $slug . '_page_my-calendar-categories' === $id ) {
