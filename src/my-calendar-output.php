	...
	if ( ( ! $has_link && 'mini' !== $type && 'list' !== $type || ( ! $has_link && 'list' === $type && 'true' === mc_get_option( 'list_link_titles' ) || 'card' === $type ) ) && ! $omit_link ) {
		if ( 'true' === $open_uri || 'card' === $type ) {
			...
			if ( ( 'modal' === $gridtype && 'calendar' === $type ) || ( 'modal' === $listtype && 'list' === $type ) ) {
				...
				$classes = 'mc-modal mc-button mc-text-button button-link';
			} else {
...
function mc_get_event_image( $event, $data, $size = '' ) {
	...
	if ( is_numeric( $event->event_post ) && 0 !== (int) $event->event_post && ( isset( $data[ $default_size ] ) && '' !== $data[ $default_size ] ) ) {
		...
		$image = ( '' !== $event->event_image && null !== $event->event_image && 0 !== (int) $event->event_image ) ? "<img src='" . esc_url( $event->event_image ) . "' alt='" . esc_attr( $alt ) . "' class='mc-image photo' />" : '';
	}
	$return = true;
...
function mc_edit_panel( $html, $event, $type, $time, $date ) {
	...
	if ( ! mc_show_details( $time, $type ) ) {
		...
		if ( mc_can_edit_event( $event ) && mc_get_option( 'remote' ) !== 'true' ) {
			...
			$edit      = "	<div class='mc_edit_links'><button type='button' class='mc-toggle-button mc-toggle-edit has-popup' aria-haspopup='true' aria-expanded='false' aria-controls='mc-edit-$control-$type'><span class='dashicons dashicons-edit' aria-hidden='true'></span>" . __( 'Edit', 'my-calendar' ) . "</button><ul id='mc-edit-$control-$type'>";
			/**
							...
							if ( ( $is_weekend && $show_weekends ) || ! $is_weekend ) {
								...
								if ( 'list' === $params['format'] ) {
									...
									$body .= "<$td id='$params[format]-$date_is'$ariacurrent class='mc-events $dateclass $weekend_class $monthclass $events_class day-with-date'>
										<div class='mc-date-container$has_month'>
											$insert\n
											$month_heading\n
											<$element class='mc-date$trigger'>
												<span aria-hidden='true' class='mc-day-number'>$thisday_heading</span>
												<span class='screen-reader-text mc-day-date'>" . date_i18n( $date_format, strtotime( $date_is ) ) . "</span>
												$inner
											</$close>
										</div>" . $event_output . "\n</$td>\n";
								}
