...
function mc_time_html( $event, $type, $current ) {
	...
	$cur_date    = ( $type == 'list' ) ? '' : "<span class='mc-event-date dtstart' itemprop='startDate' title='" . $id_start . 'T' . $event->event_time . "' content='" . $id_start . 'T' . $event->event_time . "'>$current</span>";
				...
				$time .= "
					...
					<span class='end-time dtend'>
						<time class='value-title' datetime='" . $id_end . 'T' . $event->event_endtime . "' title='" . $id_end . 'T' . $event->event_endtime . "'>" . 
							date_i18n( $time_format, strtotime( $event->event_endtime ) ) . "
						</time>
					</span>";
			}
		}
	...
	$time .= ( $event->event_hide_end == 0 ) ? "<meta itemprop='endDate' content='" . $id_start . 'T' . $event->event_endtime . "'/>" : '';
...
function my_calendar_draw_event( $event, $type = "calendar", $process_date, $time, $template = '' ) {
	...
	$header       .= ( $type != 'single' && $type != 'list' ) ? "<$hlevel class='event-title summary$group_class' id='$uid-title'>$inner_heading</$hlevel>\n" : '';
	...
	if ( mc_show_details( $time, $type ) ) {
		...
		if ( $details === false ) {
			...
			if ( $display_more != 'false' && ! isset( $_GET['mc_id'] ) ) {
			...
			if ( $display_desc == 'true' || $type == 'single' ) {
			...
			if ( $display_short == 'true' && $type != 'single' ) {
 ...
 * @param string $prfix Prefix to append to category; varies on context.
 *
 * @return string classes
 */
...
function mc_list_title( $events ) {
	...
	$title = apply_filters( 'mc_list_event_title_hint', strip_tags( stripcslashes( $now->event_title ), mc_strip_tags() ), $now, $events ) . "<span class='mc-list-extended'>$cstate</span>";
...
function mc_calendar_params( $args ) {
	...
	$params = array(
		'name'     => $name, // not used in my_calendar(); what is it for?
		'format'   => $format,
		'category' => $category,
		'above'    => $above,
		'below'    => $below,
	...
	if ( isset( $_GET['mc_id'] ) && $source != 'widget' ) {
		...
		if ( $params['time'] == 'day' ) {
			...
			if ( $params['format'] == "calendar" || $params['format'] == "mini" ) {
				...
				$caption_heading   = ( $params['time'] != 'week' ) ? $current_header . $caption_text : $week_caption . $caption_text;
			...
			if ( $params['format'] == 'list' ) {
				...
				for ( $i = 0; $i <= 6; $i ++ ) {
					...
					if ( ( $class == 'weekend-heading' && $show_weekends ) || $class != 'weekend-heading' ) {
			...
			if ( $no_events && $params['format'] == "list" && $show_all == false ) {
				...
				do {
					...
					if ( $show_weekends || ( ! $show_weekends && ! $is_weekend ) ) {
						...
						$ariacurrent      = ( $dateclass == 'current-day' ) ? ' aria-current="date"' : '';
						...
						if ( ! empty( $events ) ) {
							...
							if ( $hide_nextmonth == true && $monthclass == 'nextmonth' ) {
								...
								if ( $params['format'] == 'mini' && $day_uri != 'false' ) {
									$event_output = ' ';
								} else {
							...
							if ( $params['format'] == 'mini' && $event_output != '' ) {
							...
							if ( ( $is_weekend && $show_weekends ) || ! $is_weekend ) {
								...
								if ( $params['format'] == "list" ) {
									...
									if ( $event_output != '' ) {
										$body .= "
											<li id='$params[format]-$date_is' $ariacurrent class='mc-events $dateclass $events_class $odd'>
												<strong class=\"event-date\">" . mc_wrap_title( date_i18n( $date_format, $start ) ) . "$title</strong>" . $event_output . "
											</li>";
										$odd = ( $odd == 'odd' ) ? 'even' : 'odd';
									}
								} else {
									$body .= "
											<$td id='$params[format]-$date_is' $ariacurrent class='$dateclass $weekend_class $monthclass $events_class day-with-date'>" . "
							...
							if ( $params['format'] != "list" ) {
								...
								$body .= "
											<$td $ariacurrent class='no-events $dateclass $weekend_class $monthclass $events_class day-with-date'>
												<span class='mc-date no-events'><span aria-hidden='true'>$thisday_heading</span><span class='screen-reader-text'>" . date_i18n( $date_format, strtotime( $date_is ) ) . "</span></span>
											</$td>\n";
							} else {
								if ( $show_all == true ) {
									$body .= "
										<li id='$params[format]-$date_is' $ariacurrent class='no-events $dateclass $events_class $odd'>
											<strong class=\"event-date\">" . mc_wrap_title( date_i18n( $date_format, $start ) ) . "</strong></li>";
									$odd = ( $odd == 'odd' ) ? 'even' : 'odd';
								}
							}
						}
...
function mc_generate_calendar_nav( $params, $cat, $start_of_week, $show_months, $main_class, $site, $date, $from ) {
		...
		if ( in_array( 'print', $used ) ) {
			...
			$print        = "
			<div class='mc-print'>
				<a href='$mc_print_url'>" . __( 'Print<span class="maybe-hide"> View</span>', 'my-calendar' ) . "</a>
			</div>";
		}
...
function mc_build_mini_url( $start, $category, $events, $args, $date ) {
	...
	if ( $open_day_uri == 'true' || $open_day_uri == 'false' ) {
		...
		$day_url = mc_build_url( $target, array(
				...
				'cid',
				'mc_id'
			), apply_filters( 'mc_modify_day_uri', mc_get_uri( reset( $events ), $args ) )
		);
...
function mc_get_current_date( $main_class, $cid, $params ) {
	...
	if ( isset( $_GET['month'] ) && $main_class == $cid ) {
		...
		$c_month = ( $c_m == 0 ) ? date( "m", $timestamp ) : date( "m", strtotime( $xnow . ' -1 month' ) );
	}

	$is_start_of_week = ( date( 'N', $timestamp ) == get_option( 'start_of_week' ) ) ? true : false;
...
function mc_filters( $args, $target_url, $ltype ) {
	...
	foreach ( $qsa as $name => $argument ) {
		$name     = esc_attr( strip_tags( $name ) );
		$argument = esc_attr( strip_tags( $argument ) );
		if ( ! ( $name == 'access' || $name == 'mcat' || $name == 'loc' || $name == 'ltype' || $name == 'mc_id' ) ) {
