...
function mc_edit_panel( $html, $event, $type, $time, $date ) {
	...
	if ( ! mc_show_details( $time, $type ) ) {
		...
		if ( mc_can_edit_event( $event ) && mc_get_option( 'remote' ) !== 'true' ) {
			...
			$edit      = "	<div class='mc_edit_links'><button type='button' class='mc-toggle-button mc-toggle-edit has-popup' aria-expanded='false' aria-controls='mc-edit-$control-$type'><span class='dashicons dashicons-edit' aria-hidden='true'></span>" . __( 'Edit', 'my-calendar' ) . "</button><ul id='mc-edit-$control-$type'>";
			/**
